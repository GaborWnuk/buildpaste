...
/**
 * The block-name table shared with the buildpaste.net backend.
 *
...
object BlockTable {
	 ...
	 * as a JSON number that arrived as `12.0`) or an already-namespaced name.
	 */
	fun resolve(idOrName: String): String {
		val numeric = idOrName.toFloatOrNull()?.let { Math.round(it) }
		return if (numeric != null) nameOf(numeric) ?: idOrName else idOrName
	}
}
